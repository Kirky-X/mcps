        ...
        try:
            ...
            if fuzzy_search and len(docs) > 1:
                ...
                if exact_matches:
                    ...
                    selected_doc = docs[0]
                    self.logger.info(
                        f"Selected best match: {selected_doc.get('g')}:{selected_doc.get('a')} (priority score: {get_priority_score(selected_doc)})")
                else:
                    ...
                    selected_doc = docs[0]
                    self.logger.info(
                        f"Selected most popular match: {selected_doc.get('g')}:{selected_doc.get('a')} (usageCount: {selected_doc.get('usageCount', 0)})")
            else:
